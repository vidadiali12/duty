...
export default function UpdateAndAddPersonnel({ item, setItem, ranks, setCreateAndUpdate, apiOpe, typeOpe, setResponseRequest }) {
    ...
    useEffect(() => {
        const callRols = async () => {
            try {
                ...
                const resRols = await api.get("/admin/role/getAllRole", { headers: { Authorization: `Bearer ${token}` } });
    ...
    const validationOfForm = () => {
        ...
        Object.entries(formKey).forEach(([key, label]) => {
            if (!form[key]?.toString().trim()) {
                countOfBlankFields++;
                newErrs[key] = `${label} Boş qala bilməz`;
            }
        });
        setErrs(newErrs);
    ...
    return (
        <div className="update-personnel-wrapper">
            <div className="update-personnel-card">
                <h2>{typeOpe === "editOpe" ? "İstifadəçi Yeniləmə" : "İstifadəçi Yaradılması"}</h2>
                <form onSubmit={handleSubmit}>
                    <div className="box-of-fields">
                        ...
                        {errs.username && <span className="err-of-personnal-fields">{errs.username}</span>}
                    </div>

                    {!item && (
                        <div className="box-of-fields">
                            <label>Parol</label>
                            <input name="password" value={form.password} onChange={handleChange} placeholder="Parol: " autoComplete="off" type={!eye ? "password" : "text"} />
                            {!eye ? <FiEye onClick={() => setEye(!eye)} className="eye-icon" /> : <FiEyeOff onClick={() => setEye(!eye)} className="eye-icon" />}
                            {errs.password && <span className="err-of-personnal-fields">{errs.password}</span>}
                        </div>
                    )}
                    ...
                    <div className="box-of-fields">
                        ...
                        {errs.email && <span className="err-of-personnal-fields">{errs.email}</span>}
                    </div>

                    <div className="box-of-fields">
                        <label>FIN</label>
                        <input name="fin" value={form.fin} onChange={handleChange} placeholder="Fin: " />
                        {errs.fin && <span className="err-of-personnal-fields">{errs.fin}</span>}
                    </div>

                    <div className="box-of-fields">
                        <label>Ad</label>
                        <input name="name" value={form.name} onChange={handleChange} placeholder="Ad: " />
                        {errs.name && <span className="err-of-personnal-fields">{errs.name}</span>}
                    </div>

                    <div className="box-of-fields">
                        <label>Soyad</label>
                        <input name="surname" value={form.surname} onChange={handleChange} placeholder="Soyad: " />
                        {errs.surname && <span className="err-of-personnal-fields">{errs.surname}</span>}
                    </div>

                    <div className="box-of-fields">
                        <label>Ata adı</label>
                        <input name="fatherName" value={form.fatherName} onChange={handleChange} placeholder="Ata adı: " />
                        {errs.fatherName && <span className="err-of-personnal-fields">{errs.fatherName}</span>}
                    </div>

                    <div className="box-of-fields">
                        <label>Rütbə</label>
                        <select name="rankId" value={form.rankId} onChange={handleRank}>
                            <option value="">Rütbə seç</option>
                            {ranks?.map((r, index) => <option key={index} value={r.id}>{r.name}</option>)}
                        </select>
                        {errs.rankId && <span className="err-of-personnal-fields">{errs.rankId}</span>}
                    </div>

                    <div className="box-of-fields">
                        <label>Vəzifə</label>
                        <input name="position" value={form.position} onChange={handleChange} placeholder="Vəzifə: " />
                        {errs.position && <span className="err-of-personnal-fields">{errs.position}</span>}
                    </div>

                    <div className="box-of-fields">
                        <label>Durum</label>
                        <select name="roleId" value={form.roleId} onChange={handleRank}>
                            <option value="">Durum seç</option>
                            {rols?.map((r, index) => <option key={index} value={r.id}>{r.name}</option>)}
                        </select>
                        {errs.roleId && <span className="err-of-personnal-fields">{errs.roleId}</span>}
                    </div>
                    ...
                    <div className="buttons">
                        <button type="submit" disabled={loading}>
                            {loading ? "Yüklənir..." : typeOpe === "editOpe" ? "Yenilə" : "Hesab Yarat"}
                        </button>
                        <button type="button" onClick={() => { setCreateAndUpdate(false); setItem(null); }} className="cancel">
                            Ləğv et
                        </button>
                    </div>
                </form>
